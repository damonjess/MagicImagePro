...
class ObjectRemover(context: Context) : TFLiteModel(context, "lama_dilated-tflite-float.tflite") {
    ...
    suspend fun removeObject(image: Bitmap, maskBitmap: Bitmap): Bitmap = withContext(Dispatchers.Default) {
        ...
        // 1. Dynamic Context Multiplier (Fixes "Context Starvation" Fog)
        ...
        val paddingMultiplier = when {
            objectRatio > 0.35f -> 3.2f  // Large object: give maximum surrounding context
            objectRatio > 0.15f -> 2.4f  // Medium object (e.g., subject in frame)
            else -> 1.8f                 // Small object: keep crop tight for sharpness
        }
        ...
        // 3. Circular Mask Dilation (Eats rough borders & shadows)
